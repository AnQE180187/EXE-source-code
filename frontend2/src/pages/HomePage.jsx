...
import { ArrowRight, Search, Users, CalendarCheck, MessageSquare, Award } from 'lucide-react';
import './HomePage.css';

const featuredEvents = [
  { id: 1, title: 'Hội thảo Công nghệ Tương lai', category: 'Công nghệ',
    imageUrl: 'https://images.unsplash.com/photo-1518770660439-4636190af475?auto=format&fit=crop&w=1170&q=80' },
  { id: 2, title: 'Workshop Thiết kế UI/UX Chuyên sâu', category: 'Thiết kế',
    imageUrl: 'https://images.unsplash.com/photo-1558655146-d09347e92766?auto=format&fit=crop&w=764&q=80' },
  { id: 3, title: 'Lễ hội Âm nhạc Đa sắc màu', category: 'Âm nhạc',
    ...
    <div className="home-page page-fade">
      ...
      <section className="page-section reveal fade-up">
        ...
        <div className="featured-events-grid">
          {featuredEvents.map((event, i) => (
            <Link
              to={`/events/${event.id}`} // điều hướng hợp lý hơn
              key={event.id}
              className="event-card reveal zoom-in"
      ...
      <section className="page-section how-it-works reveal fade-up">
        ...
        <div className="steps-grid">
          {[
            { icon: <Search size={32} />, title: '1. Tìm kiếm', desc: 'Duyệt qua danh sách hoặc tìm kiếm sự kiện theo sở thích, địa điểm và thời gian.' },
            { icon: <CalendarCheck size={32} />, title: '2. Đăng ký', desc: 'Chọn sự kiện bạn yêu thích và đăng ký tham gia chỉ với vài cú nhấp chuột.' },
            { icon: <Users size={32} />, title: '3. Kết nối', desc: 'Tham gia diễn đàn, tìm bạn đồng hành và trò chuyện trước thềm sự kiện.' },
            { icon: <Award size={32} />, title: '4. Trải nghiệm', desc: 'Tận hưởng những khoảnh khắc tuyệt vời và chia sẻ kỷ niệm của bạn.' },
          ].map((s, i) => (
      ...
      <section className="page-section reveal fade-up">
        ...
        <div className="forum-preview">
          <ul className="post-list">
            {recentPosts.map((post, i) => (
              <li key={post.id} className="post-item reveal fade-right" style={{ transitionDelay: `${i * 80}ms` }}>
                <Link to="/forum" className="post-item__link hover-lift">
          ...
          <div className="forum-preview__cta reveal fade-up">
      ...
      <section className="cta-section reveal fade-up gradient-shine">
        <div className="cta-section__content">
          ...
          <Link to="/register" className="cta-section__button btn-press">Đăng ký ngay</Link>
        </div>
      </section>
    </div>
  );
